...
_SALVAGE_PENDING_NAME = "salvage-pending.json"
...
class RecoveryLeg:
    """One device range requested to reconcile a preserved partial attempt."""
    ...
    def next_sequence(self) -> int:
        return self.start_sequence + self.packet_count
...
class AttemptDescriptor:
    ...
    mode: str = "durable"
    recovery_leg: RecoveryLeg | None = None
...
def _validate_descriptor(descriptor: AttemptDescriptor, path_id: str) -> None:
    ...
    if descriptor.mode not in {"durable", "streaming"}:
        raise AttemptStateError("attempt descriptor mode is invalid")
    ...
    leg = descriptor.recovery_leg
    if leg is not None:
        _validate_int(leg.start_sequence, "recovery start_sequence")
        _validate_count(leg.packet_count)
        if descriptor.read_begin_start is None or leg.start_sequence < descriptor.start_sequence:
            raise AttemptStateError("attempt descriptor recovery leg is outside requested range")
        if leg.next_sequence > descriptor.start_sequence + descriptor.packet_count:
            raise AttemptStateError("attempt descriptor recovery leg exceeds requested range")
...
def _mode_from_json(raw: dict[str, object]) -> str:
    value = raw.get("mode", "durable")
    if not isinstance(value, str):
        raise ValueError("mode")
    return value


def _recovery_leg_from_json(raw: dict[str, object]) -> RecoveryLeg | None:
    value = raw.get("recovery_leg")
    if value is None:
        return None
    if not isinstance(value, dict):
        raise ValueError("recovery_leg")
    ...
    if isinstance(count, bool) or not isinstance(count, int):
        raise ValueError("recovery_leg")
    return RecoveryLeg(start, count)
...
def _descriptor_from_json(raw: dict[str, object]) -> AttemptDescriptor:
    return AttemptDescriptor(
        ...
        mode=_mode_from_json(raw),
        recovery_leg=_recovery_leg_from_json(raw),
    )
